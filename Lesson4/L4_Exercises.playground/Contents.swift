...
newMountainViewRent = oldMountainViewRent.flatMap{ $0 + $0 * 0.2 }
//: ### Exercise 3
...
for (food, approved) in polarizingFoods {
    if approved {
        print("\(food), yum!")
    } else {
        print("\(food), yuck!")
...
rapperPhoneNumbers.flatMap{ $1.replacingOccurrences(of: "(415)", with: "(510)") }
//: ## Switch Statements
...
case let (me, you) where me == you: print("It's a tie!")
case (.rock, .scissors), (.scissors, .rock): print("Rock crushes Scissors.")
case (.paper, .scissors), (.scissors, .paper): print("Scissors cut Paper.")
case (.paper, .rock), (.rock, .paper): print("Paper covers Rock.")
default: print("")
}
//: ### Exercise 7
